...
var search = async (event) => {
  try {
    ...
    const typeFilter = queryParams.type?.toLowerCase() || "any";
    const where = {};
    if (typeFilter !== "any") {
      const styleMap = {
      ...
      const danceStyle = styleMap[typeFilter];
      if (danceStyle) {
        where.style = danceStyle;
      } else {
        return {
          statusCode: 400,
          headers: {
          ...
          body: JSON.stringify({
            error: `Invalid type filter. Must be one of: salsa, bachata, reggaeton, any`
          })
        };
      }
    }
